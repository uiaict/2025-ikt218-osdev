...
void initIdt() {
    ...
    // Initialize idt_entries with all 0 entries. We do this since we are not going to be using all 256 slots
    memset(&idt_entries,0,sizeof(struct idt_entry_struct) * 256);

    // Program Primary and Secondary PIC
    // Primary: 0x20 for commands and 0x21 for data
    // Secondary: 0xA0 for commands and 0xA1 for data
    outPortB(0x20,0x11);
    outPortB(0xA0,0x11);
