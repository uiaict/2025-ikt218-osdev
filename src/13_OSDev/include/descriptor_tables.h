...
// Define the IDT entry structure. Defines how each entry looks in memory  - needed for the IDT
struct idt_entry_t {
  uint16_t base_low;
  uint16_t selector;
  uint8_t zero;
...
// Define the GDT and IDT pointers Used to tell the CPU where the IDT starts and its size
struct gdt_ptr_t {
  uint16_t limit;
  uint32_t base;
} __attribute__((packed));
