...
void initIdt() {
    ...
    setIdtGate(128, (uint32_t)isr128, 0x08, 0xEE); // DPL 3 for user-mode syscalls
    setIdtGate(177, (uint32_t)isr177, 0x08, 0x8E); // Keeping 177 for now, DPL 0
...
void setIdtGate(uint8_t num, uint32_t base, uint16_t sel, uint8_t flags) {
    ...
    idt_entries[num].flags = flags; // Removed | 0x60
}
...
void isr_handler(struct InterruptRegisters* regs) {
    ...
    if (regs->int_no < 32) {
        ...
        print("\n");
        // Halt only for fatal exceptions
        if (regs->int_no == 0 || regs->int_no == 8 || regs->int_no == 14) { // Divide-by-Zero, Double Fault, Page Fault
...
void irq_handler(struct InterruptRegisters* regs) {
    ...
    if (regs->int_no >= 40) {
        outPortB(0xA0, 0x20); // EOI to secondary PIC
    }
    outPortB(0x20, 0x20); // EOI to primary PIC
}