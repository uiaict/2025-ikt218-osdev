#include <memory.h>
#include <print.h>    // Add this for printf
...
{
    last_alloc = (uint32_t)kernel_end + 0x1000;  // Fix: Cast pointer to integer before arithmetic
...
void pfree(void *mem)
{
    uint32_t mem_addr = (uint32_t)mem;  // Fix: Cast pointer to integer for comparison
    if(mem_addr < pheap_begin || mem_addr > pheap_end) return;

    // Determine the page ID
    uint32_t ad = mem_addr - pheap_begin;
    ad /= 4096;
