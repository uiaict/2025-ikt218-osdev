...
// Reference to kernel_directory from paging.c
extern page_directory_t *kernel_directory;

// The kernel's heap
heap_t *kheap = 0;

// The kernel's heap start address for placement allocator
...
void init_kernel_memory(u32int start)
{
    ...
    monitor_write("Kernel memory initialized\n");
}
   ...
   while (iterator < heap->index.size)
   {
       ...
       if (page_align > 0)
       {
           ...
           if (((location + sizeof(header_t)) & 0xFFFFF000) != 0)
               offset = 0x1000 /* page size */  - ((location + sizeof(header_t)) & 0xFFF);
...
// Custom comparison function for headers
...
static void expand(u32int new_size, heap_t *heap)
{
   ...
   if (heap->start_address + new_size > heap->max_address)
       new_size = heap->max_address - heap->start_address;
...
void free(void *p, heap_t *heap)
{
   ...
   {
       monitor_write("Header magic is wrong!\n");
       return;
   }
   if (footer->magic != HEAP_MAGIC)
   {
       monitor_write("Footer magic is wrong!\n");
       return;
   }
...
// Implementation of kmalloc using placement allocator or heap
static void* kmalloc_int(u32int sz, int align, u32int *phys)
{
    // If the heap is initialized, use it
    if (kheap != 0)
    {
    ...
    {
        // Align the placement address if needed
...
// Placeholder for free function - now uses the heap's free
void kfree(void *p)
{
    if (kheap != 0)
        free(p, kheap);
