...
#include "libc/util.h"    // for inPortB and outPortB
#include "libc/PitTimer.h"   // for delay()
...
void stopSound() {
    uint8_t port_val = inPortB(PC_SPEAKER_PORT);     // Read port 0x61
    port_val &= ~0x02;                     // Clear bit 1 (speaker data)
    outPortB(PC_SPEAKER_PORT, port_val);             // Write back to port 0x61
}
