...
void timer_handler(void) {
    tick_count++;
    pic_send_eoi(0);  // Send End of Interrupt signal
}
...
void init_programmable_interval_timer(void) {
    ...
    // Configure PIT mode
    outb(PIT_COMMAND_PORT, PIT_CHANNEL0 | PIT_LOHI | PIT_MODE2);
    io_wait();
    ...
    display_write_decimal(TARGET_FREQUENCY);
    display_write(" Hz\n");
 ...
 * (low CPU usage, relies on timer interrupts)
 * 
 * @param milliseconds Number of milliseconds to sleep
 */
