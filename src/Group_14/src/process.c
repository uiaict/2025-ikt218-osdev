 ...
 */
 ...
 static bool allocate_kernel_stack(pcb_t *proc)
 {
     ...
     if (kstack_virt_base < KERNEL_STACK_VIRT_START || kstack_virt_end > KERNEL_STACK_VIRT_END || kstack_virt_end <= kstack_virt_base) {
          terminal_printf("  [Process] Error: Kernel stack virtual address space exhausted or invalid (Base=%#x, End=%#x).\n", (unsigned long)kstack_virt_base, (unsigned long)kstack_virt_end);
          // Cleanup frames
     ...
     terminal_printf("  Allocated kernel stack VIRTUAL range: [%#x - %#x)\n",
                     (unsigned long)kstack_virt_base, (unsigned long)kstack_virt_end);
     ...
     terminal_printf("  Kernel stack mapped: PhysBase=%#x, VirtBase=%#x, VirtTop=%p\n",
                                  ...
                                  size_t zero_padding)
 {
     ...
     PROC_DEBUG_PRINTF("Calling paging_temp_map for P=%#x\n", (unsigned long)frame_paddr);
     void* temp_vaddr = paging_temp_map(frame_paddr);
     if (temp_vaddr == NULL) {
         terminal_printf("[Process] copy_elf_segment_data: ERROR: paging_temp_map failed (paddr=%#x).\n", (unsigned long)frame_paddr);
         return -1;
     }
                                     ...
                                     uintptr_t *initial_brk)
 {
     ...
     PROC_DEBUG_PRINTF("File read: size=%u bytes, buffer=%p\n", (unsigned long)file_size, file_data);
         ...
         ehdr->e_entry == 0)                         // Must have an entry point
     {
      ...
      if (ehdr->e_entry >= KERNEL_VIRT_BASE) {
          terminal_printf("[Process] load_elf: Warning: Entry point %#x is in kernel space for '%s'.\n", (unsigned long)ehdr->e_entry, path);
          // Allow for now, but highly suspicious for a user executable
      }
 
     *entry_point = ehdr->e_entry;
     terminal_printf("  ELF Entry Point: %#x\n", (unsigned long)*entry_point);
     ...
     for (Elf32_Half i = 0; i < ehdr->e_phnum; i++) {
         ...
         {
             terminal_printf("  -> Error: Invalid segment %d geometry or placement for '%s'. V=%#x Memsz=%u Filesz=%u Offset=%u\n",
                             (int)i, path, (unsigned long)phdr->p_vaddr, (unsigned)phdr->p_memsz, (unsigned)phdr->p_filesz, (unsigned)phdr->p_offset);
             goto cleanup_load_elf;
         }
 
         terminal_printf("  Segment %d: VAddr=%#x, MemSz=%u, FileSz=%u, Offset=%#x, Flags=%c%c%c\n",
         ...
         terminal_printf("  -> VMA [%#x - %#x), VMA Flags=%#x, PageProt=%#x\n",
         ...
         for (uintptr_t page_v = vm_start; page_v < vm_end; page_v += PAGE_SIZE) {
             ...
             PROC_DEBUG_PRINTF("    Allocated frame P=%#x\n", (unsigned long)phys_page);
         ...
         PROC_DEBUG_PRINTF("  Segment %u processed. highest_addr_loaded=%#x\n", (unsigned)i, (unsigned long)highest_addr_loaded);
     ...
     *initial_brk = PAGE_ALIGN_UP(highest_addr_loaded);
     ...
     terminal_printf("  ELF load complete. initial_brk=%#x\n", (unsigned long)*initial_brk);
     result = 0; // Success
 ...
 cleanup_load_elf:
     ...
     if (phys_page != 0) {
         PROC_DEBUG_PRINTF("Freeing leftover phys_page %#x\n", (unsigned long)phys_page);
         put_frame(phys_page);
     }
 ...
 static void prepare_initial_kernel_stack(pcb_t *proc) {
     ...
     *kstack_ptr = proc->entry_point;
     PROC_DEBUG_PRINTF("Pushed EIP = %#x at %p\n", (unsigned long)*kstack_ptr, kstack_ptr);
     ...
     terminal_printf("  Kernel stack prepared for IRET. Final K_ESP = %#x\n", (unsigned long)proc->kernel_esp_for_switch);
 ...
 pcb_t *create_user_process(const char *path)
 {
     ...
     proc->pid = next_pid++;
     PROC_DEBUG_PRINTF("PCB allocated at %p, PID=%u\n", proc, (unsigned long)proc->pid);
     ...
     terminal_printf("  Allocated PD Phys: %#x for PID %u\n", (unsigned long)pd_phys, (unsigned long)proc->pid);
 
     // --- Initialize Page Directory ---
     PROC_DEBUG_PRINTF("Step 3: Initialize Page Directory (PD Phys=%#x)\n", (unsigned long)pd_phys);
     ...
     {
         PROC_DEBUG_PRINTF("  Setting up Heap VMA at %#x\n", (unsigned long)initial_brk_addr);
     ...
     // User Stack VMA
     {
         ...
         terminal_printf("  Inserting User Stack VMA [%#x - %#x) (Grows Down) for PID %u\n",
 ...
 fail_create:
     ...
     if (initial_stack_phys_frame != 0 && !initial_stack_mapped) {
          terminal_printf("  Freeing unmapped initial user stack frame P=%#x\n", (unsigned long)initial_stack_phys_frame);
     ...
     if (proc) {
         ...
         PROC_DEBUG_PRINTF("  Calling destroy_process for partially created PID %u\n", (unsigned long)proc->pid);
         destroy_process(proc);
     } else {
         // If PCB allocation failed but PD was allocated
         if (pd_phys) {
              PROC_DEBUG_PRINTF("  Freeing PD frame P=%#x (PCB allocation failed)\n", (unsigned long)pd_phys);
             put_frame(pd_phys);
         }
     }
     ...
     if (pcb->kernel_stack_vaddr_top != NULL) {
         ...
         for (uintptr_t v_addr = stack_base; v_addr < stack_top; v_addr += PAGE_SIZE) {
             ...
             if (paging_get_physical_address((uint32_t*)g_kernel_page_directory_phys, v_addr, &phys_addr) == 0) {
                 if (phys_addr != 0) {
                     PROC_DEBUG_PRINTF("   Found P=%#x. Calling put_frame.\n", (unsigned long)phys_addr);
