 ...
 * keymap.c
 * 
 * Implements keyboard layout loading for a 32-bit x86 OS.
 * Supports multiple layouts including US QWERTY, UK QWERTY, Dvorak, and Colemak.
 *
 * Each layout is defined as a static constant array mapping PS/2 Set 1 scancodes (0x00 to 0x7F)
 * to a KeyCode. Unmapped or unsupported scancodes default to KEY_UNKNOWN.
 *
 * The keymap_load() function selects the appropriate layout and passes it to the keyboard driver.
 *
 * Note: Ensure that the KeyCode enum and helper functions (e.g., keyboard_set_keymap())
 * are defined in keyboard.h.
 */
 ...
 const uint16_t keymap_uk_qwerty[128] = {
     ...
     [0x0D] = KEY_UNKNOWN,  // Often '£' on UK keyboards
     [0x0E] = '\b',
     [0x0F] = '\t',
     [0x10] = 'q',
     [0x11] = 'w',
     ...
     [0x27] = ';',
     [0x28] = '\'',
     [0x29] = '#',          // UK keyboards often map '#' here.
     [0x2A] = KEY_LEFT_SHIFT,
     [0x2B] = '\\',
     [0x2C] = 'z',
     [0x2D] = 'x',
 ...
 const uint16_t keymap_dvorak[128] = {
     ...
     [0x14] = 'y',           // T remaps to y
     [0x15] = 'f',           // Y remaps to f
     [0x16] = 'g',           // U remaps to g
     [0x17] = 'c',           // I remaps to c
     [0x18] = 'r',           // O remaps to r
     [0x19] = 'l',           // P remaps to l
     [0x1A] = '/',           // [ remaps to /
     [0x1B] = '=',           // ] remaps to =
     [0x1C] = '\n',
     [0x1D] = KEY_CTRL,
     ...
     [0x2B] = '\\',
     [0x2C] = ';',           // Z remaps to ;
     [0x2D] = 'q',           // X remaps to q
     [0x2E] = 'j',           // C remaps to j
     [0x2F] = 'k',           // V remaps to k
     [0x30] = 'x',           // B remaps to x
     [0x31] = 'b',           // N remaps to b
     [0x32] = 'm',           // M remains m
     [0x33] = ',',           // , remains
     [0x34] = '.',           // . remains
     [0x35] = '/',           // / remains
  ...
  * In Colemak, many keys remain in their QWERTY positions; only select keys are rearranged.
  */
  ...
  *
  * This function selects one of the predefined keymaps and updates the
  * keyboard driver’s active keymap accordingly.
  *
