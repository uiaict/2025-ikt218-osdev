 ...
 */
 ...
 // If this static assert continues to fail, PAGE_SIZE might not be treated as a
 // constant expression, or the array access itself might be the issue.
 // Consider replacing with a runtime check in init if necessary.
 ...
 void percpu_kmalloc_init(void) {
    ...
    for (int cpu = 0; cpu < MAX_CPUS; cpu++) {
        ...
        for (size_t i = 0; i < NUM_PERCPU_SIZE_CLASSES; i++) {
            ...
            // Use snprintf (needs <libc/stdio.h>)
            ...
            if (!cpu_allocators[cpu].slab_caches[i]) {
                 ...
                 success = false;
                 // Should we clean up already created caches on partial failure?
            } else {
                 // terminal_printf("  - Created slab cache '%s' (obj size %u)\n", cache_name, cache_obj_size);
            }
        }
    }
    if (success) {
