 ...
 * Includes public API for mounting/unmounting the root filesystem.
 *
 ...
 * - Fixed printf format specifier for file->flags (uint32_t -> %lx).
 * - Removed use of undeclared O_DIRECTORY in self-test.
 * - Added definitions for vfs_mount_root and vfs_unmount_root.
 */
 ...
 static int vfs_mount_internal(const char *mp, const char *fs, const char *dev); // Renamed for clarity
 static int vfs_unmount_internal(const char *mp); // Renamed for clarity
 static int vfs_unmount_entry(mount_t *mnt); // Actual unmount logic per entry
 ...
 /*---------------------------------------------------------------------------
  * Mount / Unmount Operations (Internal and Public API)
  *---------------------------------------------------------------------------*/
 ...
 static int vfs_unmount_internal(const char *mp) {
       ...
       // Perform the actual unmount using the helper
  ...
  * @param fs_type Filesystem type name (e.g., "fat32").
  * @param dev Device identifier string (e.g., "/dev/hda1").
  * @return FS_SUCCESS on success, negative error code otherwise.
  */
 int vfs_mount_root(const char *mp, const char *fs_type, const char *dev) {
     ...
     // Call the internal mount function
 ...
 int vfs_unmount_root(void) {
     ...
     // Call the internal unmount function for the root path
 ...
 int vfs_shutdown(void) {
       ...
       while ((current_mount = mount_table_get_head()) != NULL && unmount_attempts < max_attempts) {
           ...
           if (current_mount->mount_point) {
               ...
               // Let's try unmount_entry which handles NULL checks inside.
           }
 
 
           VFS_LOG("Attempting shutdown unmount for '%s'...", mp_copy);
           int result = vfs_unmount_entry(current_mount); // Use helper which removes from table
           if (result != FS_SUCCESS) {
