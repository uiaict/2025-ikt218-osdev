                            ...
                            size_t heap_size)
{
     ...
     // <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<< FIX START >>>>>>>>>>>>>>>>>>>>>>>>>>>>>
     // Ensure PDE[1022] (covering PAGING_TEMP_VADDR) exists before paging activation.
     terminal_write("  Pre-allocating Page Table for Temporary Mapping Area (PDE 1022)...\n");
     uintptr_t temp_pt_phys = paging_alloc_early_frame_physical(); // Use early allocator
     if (!temp_pt_phys) {
         KERNEL_PANIC_HALT("Failed to allocate PT frame for temporary mapping area!");
     }
     // Frame is zeroed by early allocator.
     // Set PDE[1022] in the physical PD being built.
     volatile uint32_t* pd_phys_ptr = (volatile uint32_t*)page_directory_phys;
     uint32_t temp_pde_flags = PAGE_PRESENT | PAGE_RW | PAGE_NX_BIT; // Kernel RW, Present, NX ok
                             ...
                             bool use_large_page)
{
    ...
    if (is_current_pd) {
        ...
        if (use_large_page) {
            ...
            if (current_pde & PAGE_PRESENT) {
                ...
                return KERN_EEXIST; // Use KERN_EEXIST or similar
            }
        ...
        uint32_t* target_pt_virt_temp = NULL; // Changed: Use one temp PT mapping pointer
        uintptr_t pt_phys = 0;
        ...
        if (use_large_page) {
            // ... (Handling for 4MB pages remains the same) ...
             uint32_t new_pde_val_4mb = (aligned_paddr & PAGING_PDE_ADDR_MASK_4MB) | pde_final_flags;
             if (pde & PAGE_PRESENT) {
                 // Line 1242 Fix: %lu, %p
             ...
             // No temporary PT mapping needed for 4MB pages
             goto cleanup_other_pd;

        } else {
            // --- Handle 4KB page for other PD ---
            uint32_t* temp_pt_vaddr = NULL; // Vaddr used for temporary PT mapping
            ...
            if (pde & PAGE_PRESENT) {
                ...
                memset(temp_pt_vaddr, 0, PAGE_SIZE);
                // --- DO NOT UNMAP YET ---
                // paging_temp_unmap_vaddr(temp_pt_vaddr); // <-- REMOVED premature unmap
                ...
                target_pd_virt_temp[pd_idx] = pde_value_to_write; // Write the PDE *before* unmapping PT
                // ********************** FIX END **********************
            }
            ...
            } else { // We already mapped it when allocating
                 target_pt_virt_temp = temp_pt_vaddr;
            }
            ...
            if (current_pte & PAGE_PRESENT) {
                ...
                if (existing_phys == (aligned_paddr & PAGING_ADDR_MASK)) {
                    ...
                    if (pt_allocated_here) {
                        ...
                        put_frame(pt_phys);
                        // Need to unmap the temp PT mapping if it's still active
                        if(temp_pt_vaddr) {
                            paging_temp_unmap_vaddr(temp_pt_vaddr);
                            temp_pt_vaddr = NULL; // Prevent double unmap in cleanup
                        }
                    }
                }
            } else {
            ...
            // --- Unmap the temporary PT mapping (whether existing or newly allocated) ---
            if (target_pt_virt_temp) { // Check if it was successfully mapped
                paging_temp_unmap_vaddr(target_pt_virt_temp);
            }
    ...
    cleanup_other_pd:
        ...
        if (target_pd_virt_temp) {
            paging_temp_unmap_vaddr(target_pd_virt_temp); // Use the safe unmap
        }
        return ret;
                                 ...
                                 uintptr_t *paddr_out) // Output physical address
 {
     ...
     *paddr_out = 0; // Default output to 0
     ...
     // --- Case 2: Looking up in a DIFFERENT Page Directory ---
         ...
         // Original logic using temporary mappings (ensure it uses paging_temp_map_vaddr/unmap safely)
         ...
         paging_temp_unmap_vaddr(target_pd_virt_temp);
         return ret;
     }
 }
          ...
          if (pde & PAGE_PRESENT) {
              if (pde & PAGE_SIZE_4MB) {
                  ...
                  if (target_pt_virt_temp) {
                       ...
                       paging_temp_unmap_vaddr(target_pt_virt_temp); // Unmap the PT
                   } else {
 ...
 uintptr_t paging_clone_directory(uint32_t* src_pd_phys_addr) {
      ...
      for (size_t i = 0; i < KERNEL_PDE_INDEX; i++) {
          ...
          if (!dst_pt_virt_temp) {
              ...
              paging_temp_unmap_vaddr(src_pt_virt_temp); // Unmap src PT first
              error_occurred = 1; goto cleanup_clone_err;
          }
          ...
          paging_temp_unmap_vaddr(dst_pt_virt_temp);
          paging_temp_unmap_vaddr(src_pt_virt_temp);
          // Set the PDE in the destination PD
          dst_pd_virt_temp[i] = (dst_pt_phys & PAGING_ADDR_MASK) | (src_pde & PAGING_FLAG_MASK);
      }
 
  cleanup_clone_err:
      if (src_pd_virt_temp) paging_temp_unmap_vaddr(src_pd_virt_temp);
      if (dst_pd_virt_temp) paging_temp_unmap_vaddr(dst_pd_virt_temp);
 ...
 int paging_unmap_range(uint32_t *page_directory_phys, uintptr_t virt_start_addr, size_t memsz) {
     ...
     for (uintptr_t v_addr = v_start; v_addr < v_end; v_addr += PAGE_SIZE) {
         ...
         if (pde & PAGE_SIZE_4MB) {
             ...
             if (!is_current_pd) paging_temp_unmap_vaddr(target_pd_virt);
             return -1;
         }
     ...
     if (!is_current_pd) {
         paging_temp_unmap_vaddr(target_pd_virt);
     }
