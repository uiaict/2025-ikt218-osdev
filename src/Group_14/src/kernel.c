 ...
 */
 ...
 static bool init_memory_management(uint32_t mb_info_phys_addr) {
     ...
     // Allocate page directory using buddy - requires physical address
     uint32_t *initial_pd_phys = (uint32_t *)buddy_alloc(PAGE_SIZE);
     if (!initial_pd_phys) { /* ... error handling ... */ terminal_write("  [ERROR] Failed to allocate kernel page directory.\n"); return false; }
     // Map the physical address to virtual to initialize it
     // Assume kernel space is mapped starting at KERNEL_SPACE_VIRT_START
     ...
     // Pass the VIRTUAL address of the page directory to mapping functions
     if (paging_init_identity_map(initial_pd_virt, phys_mapping_size, PAGE_PRESENT | PAGE_RW) != 0) { /* ... error ... */ terminal_write("  [ERROR] Failed identity map.\n"); return false; }
     if (paging_map_range(initial_pd_virt, KERNEL_SPACE_VIRT_START, phys_mapping_size, PAGE_PRESENT | PAGE_RW) != 0) { /* ... error ... */ terminal_write("  [ERROR] Failed higher-half map.\n"); return false; }

     // Activate Paging using the PHYSICAL address
     paging_set_directory(initial_pd_virt); // Keep track of virtual address globally
     paging_activate(initial_pd_phys);     // Activate using physical address
 ...
 void main(uint32_t magic, uint32_t mb_info_phys_addr) {
     ...
     // <<< --- CHANGE THIS PATH --- >>>
     // Make sure "/hello.elf" (or your chosen path) exists on your disk image!
     const char *user_prog_path = "/hello.elf";
     // <<< --- END OF CHANGE --- >>>

     pcb_t *user_proc_pcb = create_user_process(user_prog_path);
     if (user_proc_pcb) {
         // Mark scheduler ready *before* adding the first task if PIT handler might run
         pit_set_scheduler_ready(); // Moved here
         if (scheduler_add_task(user_proc_pcb) == 0) { /* OK */
             terminal_write("  [OK] Initial user process added to scheduler.\n");
         }
         else { /* Error adding task */
             terminal_write("  [ERR] Failed to add initial process to scheduler.\n");
             destroy_process(user_proc_pcb);
         }
     } else { /* Error creating process */
          terminal_write("  [ERR] Failed to create initial user process.\n");
     }
