...
void init_idt()
{
  ...
  for (int a = 0; a < IDT_entries /*eller idt_entries*/; a++)
  {
    ...
    idt[a].sel = 0x08; // Kernel code segment selector
    idt[a].zero = 0x00;
    idt[a].flags = 0x8E; // Present, DPL=0, Type=Interrupt Gate
...
void init_interrupts()
{
  ...
  outb(0x21, 0x04); // Tell master PIC that there is a slave PIC
  outb(0xA1, 0x02); // Tell slave PIC its cascade identity

  outb(0x21, 0x01); // Set the master PIC to 8086 mode
  outb(0xA1, 0x01); // Set the slave PIC to 8086 mode

  outb(0x21, 0x00); // Unmask all IRQs on master PIC
  outb(0xA1, 0x00); // Unmask all IRQs on slave PIC
