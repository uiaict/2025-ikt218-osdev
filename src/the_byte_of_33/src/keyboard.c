...
void init_keyboard_controller(void) {
    ...
    outb(0x64, 0xAD); printf("Disabled PS/2 port 1 (0x64=0xAD)\n");
    delay(10000); // Short delay to let the controller process
    outb(0x64, 0xA7); printf("Disabled PS/2 port 2 (0x64=0xA7)\n");
    delay(10000);
    ...
    outb(0x64, 0x60); printf("Setting configuration byte (0x64=0x60)\n");
    delay(10000);
    outb(0x60, 0x41); printf("Enabled IRQ1, disabled translation (0x60=0x41)\n");
    delay(10000);

    // Enable the keyboard
    outb(0x64, 0xAE); printf("Enabled PS/2 port 1 (0x64=0xAE)\n");
    delay(10000);
    outb(0x60, 0xFF); printf("Sent reset command (0x60=0xFF)\n");
    delay(10000);

    // Wait for output buffer with timeout
