...
void print_heap_blocks() {
    ...
    while ((uint32_t)mem < last_alloc) {
        ...
        uint32_t block_start = (uint32_t)mem;
        uint32_t block_end   = block_start + sizeof(alloc_t) + block->size + 4 - 1;

        printf("Block %d at 0x%x to 0x%x | size: %d | status: %s\n",
               block_index,
               block_start,
               block_end,
               (int)block->size,
    ...
    if (block_index == 0) {
        printf("No heap blocks allocated yet.\n");
    }

    printf("[End of Heap Block Status]\n\n");
}
